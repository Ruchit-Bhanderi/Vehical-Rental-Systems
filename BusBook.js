...
import "../Users_style/BusBook.css";
...
const BusBook = () => {
  ...
  const validation = () => {
    ...
    else if (typeof bookData.name !== "undefined") {
      if (!bookData.name.match(/^[a-zA-Z]+$/)) {
    ...
    if (!bookData.from_date) {
      err.mname = "Fill the informations";
      isValid = false;
    }
    else {
      console.log("no data")
  ...
  const booking = async (e) => {
    ...
    if (getToken) {
      ...
      if (isValid) {
        ...
        console.log("reactbooking", bookingData);
  ...
  const totalamount = Difference_In_Days * bus.amount;
  ...
  let adata = feedback.filter((i) => {
    return i.Booking[0].Vehicle.includes(bus._id)
  })
  ...
  return (
    <>
      ...
      <div className="container-fuild containerbooks">
        {bus ? (<div className="row bikebook ">
          <div className="col-xl-2 col-12">
            <Card className="cardbook">
              <Card.Img variant="top" src={bus.vehicleimage} />
              <p className="model">Name : {bus.name}</p>
              <p className="model">Type : {bus.type}</p>
              <p className="model">Number : {bus.number}</p>
              <p className="model">Mileage : {bus.milege}</p>
              <p className="model">Amount : {bus.amount}</p>
              ...
              <form>
                <div className="row  justify-content-center  align-items-center ">
                  <div className="col-12 col-lg-9 col-xl-9">
                    ...
                    <button type="button" onClick={booking} class="btn bkbutton mrbtn">Book</button>

                    <NavLink exact to="/bus">
                      <button type="button" class="btn bkbutton mrbtn">Back</button>
                    </NavLink>
                  </div>
                </div>
              </form>
          ...
          <p className="bookhs2">This bus is Not avilable</p>
        )}
      ...
      {isOpen && (
        ...
        >
          <div>
            <h5 className="">This bus is Already booked, So please choose another bus..!</h5>
            <NavLink className="nav-link" exact to="/bus">
              <button className="btn lobutton">Another bus</button>
            </NavLink>
          </div>
        </ReactDialogBox>
      )}
