...
import "../Users_style/CarBook.css";
...
const CarBook = () => {
  ...
  useEffect(() => {
    if (id) {
      Axios.get(`http://localhost:8000/uservehicledata/${id}`)
        .then((res) => {
          setCar(res.data.vehicledata)
          console.log("setcar", res.data.vehicledata);
        })
    }
  }, [])
  ...
  useEffect(() => {
    Axios.get(`http://localhost:8000/feedbackdata`)
      .then((res) => {
        ...
        console.log("setcar", res.data.feedbackdata);
      })
  }, [])
  ...
  const totalamount = Difference_In_Days * car.amount;
  ...
  let adata = feedback.filter((i) => {
    return i.Booking[0].Vehicle.includes(car._id)
  })
  ...
  return (
    <>
      ...
      <div className="container-fuild containerbooks">
        {car ? (<div className="row bikebook ">
          <div className="col-xl-2 col-12">
            <Card className="cardbook">
              <Card.Img variant="top" src={car.vehicleimage} />

              <p className="model">Name : {car.name}</p>
              <p className="model">Type : {car.type}</p>
              <p className="model">Number : {car.number}</p>
              <p className="model">Mileage : {car.milege}</p>
              <p className="model">Amount : {car.amount}</p>
          ...
          <p className="bookhs2">This car is Not avilable</p>
        )}
      ...
      {isOpen && (
        <>
          ...
          >
            <div>
              <h5 className="">This Car is Already booked, So please choose another Car..!</h5>
              <NavLink className="nav-link" exact to="/car">
                <button className="btn lobutton">Another Car</button>
              </NavLink>
            </div>
          </ReactDialogBox>
        </>
